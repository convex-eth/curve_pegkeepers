...
- [x] Vyper/Solidity ABI parity: 63 functions and 13 events. Factory/interface parity: 14 functions and 5 events.
- [x] Constructor-specialized PegKeeper runtime is `22,361` bytes, `2,215` bytes below EIP-170; the compiler runtime template reported by `forge build --sizes` is `22,137` bytes.
- [x] PegKeeper full initcode including all ten static constructor arguments is `24,035` bytes, `25,117` bytes below EIP-3860.
- [x] The EIP-5202 blueprint runtime is `23,718` bytes, `858` bytes below EIP-170. The factory runtime is `4,864` bytes, `19,712` bytes below EIP-170.
- [x] Independent lifecycle/policy, expansion-preview, keeper-economics, impairment-recovery, size-remediation, frxUSD mint-adapter, and immutable-factory reviews returned PASS.
- [x] RED/GREEN deployment tests prove the direct constructor tuple and the EIP-5202 blueprint/factory tuple, fixed endpoints, snapshot roles/defaults, one-based identity, fully paused startup, future-only implementation selection, and runtime bounds.
- [x] Current-mainnet canary passed at block `25,860,039` through USDT → DAI → USDS → sUSDS, quoted `9,994.503268721919315124` crvUSD from the complete reverse route for one-tenth of the received shares, and left no residual route allowances.
...
- [ ] Confirm the default `feeReceiver`, governance `admin`, and distinct `emergencyAdmin` copied into future PegKeepers.
...
The factory owner then calls `deployPegKeeper(targetAmm, yieldToken, expansionSteps, contractionSteps)`. The factory derives target/backing assets, assigns the next index, deploys from the current blueprint, snapshots shared defaults, installs routes/configuration, rotates roles, and records the fully paused keeper atomically.

## Broadcast and activation

Do not combine deployment with activation.

1. [ ] Obtain explicit authorization for the exact blueprint hash, factory constructor tuple, factory bytecode hash, PegKeeper defaults, target AMM, yield token, and route hashes.
...
6. [ ] Confirm factory owner, implementation, defaults, and zero initial keeper count against the manifest.
...
9. [ ] Confirm every PegKeeper getter, path hash, and pause flag against the manifest.
