...
The production implementation uses Vyper `0.3.10` with the `codesize` optimizer. Its core runtime is `21,298` bytes; deployment appends the immutable shared preview-module address for an authoritative `21,330`-byte implementation runtime, `3,246` bytes below EIP-170. The stateless, keeper-identity-bound Solidity preview module is `8,201` bytes. Each EIP-1167 instance uses 55-byte initcode and a 45-byte runtime. Executable runtime/initcode, proxy-target, implementation-lock, unified deployment, and ABI-parity checks live in `test/PegKeeperV3RuntimeSize.t.sol`, `test/PegKeeperV3UnifiedDeployment.t.sol`, and the release manifest verifier.
