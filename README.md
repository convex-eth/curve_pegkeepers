...
Keeper rewards are paid from realized gross profit rather than principal. On expansion, gross profit is normalized backing stablecoin received immediately before yield deployment minus crvUSD sold. V3 pays `min(grossProfit * keeperProfitShareBps / 10_000, maxKeeperReward)` to `msg.sender` in that backing asset, then deploys the remainder and verifies the final post-reward yield position still covers principal plus the protocol margin. Fallback contraction uses the analogous crvUSD profit and pays the keeper in crvUSD. Direct buyback users receive no separate keeper reward.

Keeper execution remains fully open, but rewarded keeper functions accept no amount or minimum-output parameters. V3 calculates the largest quoted-profitable amount within inventory, deployment, and per-call bounds using fixed-route previews and a bounded binary search, then enforces path minimums, an execution-quality floor, and final realized profit. This prevents deliberate under-sizing and makes splitting around the per-call reward cap harder, though it is not an aggregate reward cap across independently profitable calls. A direct buyback user still chooses its own input and may provide a stricter personal `minTargetOut` because that user supplies crvUSD and receives the output. Open keepers can use flash liquidity around an update and may capture residual market spread; V3 accepts that limitation but requires every completed transaction to leave at least the configured protocol profit after keeper compensation.
