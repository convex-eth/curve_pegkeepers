...
A separate Chainlink alternative directly wraps the canonical proxy endpoints `frxusd-usd.data.eth` (`0x9B4a96210bc8D9D55b1908B465D8B0de68B7fF83`) and `usds-usd.data.eth` (`0xfF30586cD0F29eD462364C7e81375FC0C71219b1`). The adapter pins the proxy, feed decimals, and a mandatory maximum delay while allowing the proxy to rotate its underlying aggregator without replacing the adapter. Non-positive answers, incomplete rounds, stale data, or future timestamps revert. Chainlink lists 0.5% deviation / 24-hour heartbeat for frxUSD/USD and 0.3% deviation / 24-hour heartbeat for USDS/USD; the deployer retains a separately governed provisional 26-hour maximum delay. This alternative is implemented and tested but is not wired into the launch proposal. Governance must choose Curve EMA or direct Chainlink proxy adapters for the frxUSD and USDS checks before release evidence is finalized.
