...
The downstream expansion and yield contraction paths are separately updatable through atomic governance execution. V3 adds no contract-level route timelock because the DAO proposal already has a seven-day voting period. Emergency directional pauses remain immediate. Routes use typed Curve-swap, exact-converter, and ERC-4626 steps rather than caller-provided routers or arbitrary calldata. There is no protocol-level path-length cap; governance is trusted to configure a path that fits transaction gas and the benchmarked `minDownstreamAttemptGas`. The target, underlying, and ERC-4626 yield token are fixed for the deployment; route updates may replace venues but must preserve those endpoints. A different or non-ERC-4626 yield token requires deploying a new V3. Undeployed-backing contraction uses the designated target AMM directly and does not depend on the downstream yield route.

Configured swap paths remain exact-input and use protocol-calculated minimum outputs. The target AMM and each typed step compare measured output with that venue's same-transaction quote reduced by a configured `executionBufferBps`; the final trusted-value profit floor and downstream `maxRouteLossBps` remain independent checks. No duplicate global execution-slippage parameter or generic exact-output swap adapter is needed for the initial design. Direct buyback's exact yield-underlying remainder uses the fixed ERC-4626 vault's native `previewWithdraw`/`maxWithdraw`/`withdraw` flow with conservative share rounding and measured deltas.
