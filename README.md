...
Yield appreciation and retained execution profit are not split into separate onchain buckets. Both contribute to `protocolSurplus = max(trustedBackingValue - deployedCrvUsd, 0)`. Fee claims may withdraw either approved backing form only while the combined principal invariant remains true; action events can support offchain attribution without adding cost-basis state.
