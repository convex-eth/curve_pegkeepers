...
The caller reward is a percentage of incremental profit with no independent bound relative to deployed notional. A large favorable adjustment can therefore pay far more than the amount needed to cover transaction cost and execution risk. V3 instead constrains both the share of realized profit and the maximum reward rate on action notional.
...
V3 does not require the target crvUSD AMM spot price to remain close to its EMA. A sharp upward crvUSD move is the opportunity the expansion keeper should capture. Keeper compensation is first derived from gross realized backing-asset profit. After paying that reward and deploying the remainder, the trusted backing value of final yield-token assets must still exceed the crvUSD sold by the configured protocol margin. Balance-delta accounting, internal minimum outputs, deadlines, protocol-calculated size caps, and final profitability assertions protect execution without suppressing the intended price dislocation.
...
Keeper rewards are paid from realized gross profit rather than principal. On expansion, gross profit is normalized backing stablecoin received immediately before yield deployment minus crvUSD sold. V3 pays the lesser of `keeperProfitShareBps` of gross profit and `maxKeeperRewardBps` of crvUSD sold to `msg.sender` in that backing asset, then deploys the remainder and verifies the final post-reward yield position still covers principal plus the protocol margin. Fallback contraction uses the analogous crvUSD profit, caps reward against trusted backing spent, and pays the keeper in crvUSD. Direct buyback users receive no separate keeper reward. Because both reward bounds scale linearly with aggregate profit or notional, splitting execution across transactions cannot increase total reward beyond the aggregate bounds.

Keeper execution remains fully open, but rewarded keeper functions accept no amount or minimum-output parameters. V3 calculates the largest quoted-profitable amount within inventory, deployment, and per-call bounds using fixed-route previews and a bounded binary search, then enforces path minimums, an execution-quality floor, and final realized profit. This prevents deliberate under-sizing and improves peg effectiveness; the split-invariant reward formula independently prevents batching from bypassing compensation bounds. A direct buyback user still chooses its own input and may provide a stricter personal `minTargetOut` because that user supplies crvUSD and receives the output. Open keepers can use flash liquidity around an update and may capture residual market spread; V3 accepts that limitation but requires every completed transaction to leave at least the configured protocol profit after keeper compensation.
...
The initial timing model can use one global `lastExpansionAt`, but only a successful expansion meeting a governance-set `minExpansionAmount` resets it. A flash-liquidity actor can buy crvUSD, call expansion, and sell back to reset the timer, but V3—not the caller—chooses the largest acceptable trade and countertrades the induced price move. The actor eats the round-trip loss, fees, and V3's required entry economics. The attack is therefore paid rather than free, while genuinely distressed crvUSD can still use the higher-profit early-exit path.
