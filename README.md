...
One `expand()` call pays one reward; it does not separately reward the first USDT swap and the downstream yield route. `deployUndeployedBacking()` receives no percentage reward, and V3 does not add a separate gas reimbursement. The isolated-call gas policy is instead a safety mechanism: V3 must reserve enough outer gas to catch downstream failure and settle the fallback so a keeper cannot deliberately starve the downstream route. Exact gas-limit and settlement-reserve numbers require implementation benchmarks.
