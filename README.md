...
- **Above peg:** a permissionless keeper sells idle Factory-allocated crvUSD into a designated crvUSD/stablecoin AMM. V3 prefers to deploy the proceeds through the configured yield route, but a downstream failure or downstream-deployment pause leaves the target stablecoin as approved backing instead of reverting the peg-critical swap.
...
Undeployed backing is an intentional terminal accounting state; intermediate route assets remain transient. The core invariant counts the complete live `targetAsset` balance plus the normalized `backingAsset` value represented by the complete live `yieldToken` balance. In the USDT/sUSDS example, those components are undeployed USDT and the trusted USDS-equivalent value represented by sUSDS. Transfers of either configured token become protocol backing immediately; intermediate and arbitrary tokens remain excluded.
...
Configured paths remain exact-input and use protocol-calculated minimum outputs. The target AMM and every typed step enforce both a same-transaction quote-consistency floor and an absolute normalized-value floor using the configured buffer. A `2 bps` step therefore permits at most `2 bps` of total measured value loss, including venue fee, price impact, and pre-transaction quote manipulation; it does not add `2 bps` after `get_dy()`. The final trusted-value profit floor and downstream `maxRouteLossBps` remain independent complete-route checks. The selected launch configuration uses `3 bps` for every Curve swap, including target-AMM legs; `1 bps` for ERC-4626 deposit/redeem integer rounding; `0 bps` for exact DaiUsds conversion; and `5 bps` for every complete downstream deployment or yield-to-target maintenance route. Monetary contraction remains stricter because it must satisfy a positive exit-profit floor after all route steps. Direct buyback always sizes a conservative final-yield-token payout through `convertToShares()` and transfers that token directly; it never spends undeployed target backing. V3 prices the pre/post live yield-token position through `convertToAssets()`; the user handles any later unwrap or swap.
...
The launch uses opposite orientations of the USDC/USDT StableSwap-NG EMA for the USDC and USDT target checks. The frxUSD and USDS checks use direct Chainlink adapters around the canonical proxy endpoints `frxusd-usd.data.eth` (`0x9B4a96210bc8D9D55b1908B465D8B0de68B7fF83`) and `usds-usd.data.eth` (`0xfF30586cD0F29eD462364C7e81375FC0C71219b1`). The frxUSD adapter is reused as both the frxUSD target check and the frxUSD-denominated sfrxUSD backing check. The USDS adapter is reused as the downstream check for both sUSDS keepers. V3 first converts yield shares to frxUSD or USDS with `convertToAssets()`, then applies the capped Chainlink multiplier; favorable prices cannot over-credit backing.

Each Chainlink adapter pins the consumer-facing proxy, feed decimals, and a mandatory maximum delay while allowing the proxy to rotate its underlying aggregator without adapter replacement. Non-positive answers, incomplete rounds, stale data, or future timestamps revert. Chainlink lists 0.5% deviation / 24-hour heartbeat for frxUSD/USD and 0.3% deviation / 24-hour heartbeat for USDS/USD. The deployer and proposal currently require a separately governed provisional 26-hour maximum delay: the listed heartbeat plus two hours of grace. Typed-route quote floors, measured output, and route-loss checks remain separate from all oracle gates.
...
- [`script/DeployPegKeeperV3.s.sol`](script/DeployPegKeeperV3.s.sol): one explicit, environment-free mainnet deployment of the stateless preview module, locked implementation, immutable EIP-1167 factory, two Curve USDC/USDT adapters, and the selected frxUSD/USD and USDS/USD Chainlink adapters; after the seventh CREATE it writes every created address to `deployments/mainnet/PegKeeperV3-deployment.json`;
- [`script/PegKeeperV3ReleaseCanary.s.sol`](script/PegKeeperV3ReleaseCanary.s.sol): non-broadcasting pinned-mainnet simulation of the complete USDT → DAI → USDS → sUSDS expansion under an above-peg state, followed by a profitable sUSDS → USDS → DAI → USDT → crvUSD contraction preview under a separately created below-peg state;
...
Foundry compiles both `src/**/*.sol` and `src/**/*.vy`. The Vyper executable is pinned in `foundry.toml` to `.venv/bin/vyper`; there is no FFI compilation path. Production Vyper compilation uses the `codesize` optimizer. The implementation core is `23,201` bytes; its deployed runtime with the immutable preview-module address is `23,233` bytes, leaving `1,343` bytes below EIP-170. The stateless, keeper-identity-bound Vyper preview module is `8,056` bytes. Every minimal proxy has 55-byte initcode and a 45-byte runtime. Exact limits are asserted in `test/PegKeeperV3RuntimeSize.t.sol`, and `make check` compares the Vyper module ABI against `IPegKeeperV3PreviewModule`.
...
Runs the complete deployment against deterministic local dependencies. It verifies seven direct monotonic CREATEs from one deployer, locked implementation/preview binding, immutable factory wiring and defaults, both Curve target orientations, both selected Chainlink adapters, the explicit hardcoded mainnet configuration, and complete JSON address readback.

### `PegKeeperV3ProposalDeploymentJsonTest`

Writes the unified deployment schema and verifies that the Curve proposal loads its factory, two Curve target adapters, and selected frxUSD/USDS Chainlink adapters from that JSON instead of environment variables.
