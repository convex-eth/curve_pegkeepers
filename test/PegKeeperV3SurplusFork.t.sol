...
contract PegKeeperV3SurplusForkTest is Test {
    ...
    uint256 internal constant CLAIM_AMOUNT = 5e18;
    ...
    function test_liveFeeSplitterSurplusClaim() public {
        ...
        uint256 feeSplitterBefore = IERC20(CRVUSD).balanceOf(FEE_SPLITTER);
        uint256 idleBefore = IERC20(CRVUSD).balanceOf(address(pegKeeper));
        uint256 expansionTime = pegKeeper.last_expansion_at();
        assertGe(surplusBefore, CLAIM_AMOUNT);
        assertGe(idleBefore, CLAIM_AMOUNT);

        vm.prank(caller);
        uint256 transferred = pegKeeper.claimSurplus(CLAIM_AMOUNT);

        assertEq(transferred, CLAIM_AMOUNT);
        assertEq(IERC20(CRVUSD).balanceOf(FEE_SPLITTER) - feeSplitterBefore, CLAIM_AMOUNT);
        assertEq(idleBefore - IERC20(CRVUSD).balanceOf(address(pegKeeper)), CLAIM_AMOUNT);
        assertEq(pegKeeper.deployed_crvusd(), deployedBefore + CLAIM_AMOUNT);
        assertEq(pegKeeper.protocol_surplus(), surplusBefore - CLAIM_AMOUNT);
